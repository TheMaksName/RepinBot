...
from app.bot.FSM.FSM_user_private import MainStates, EditProfile, RegistrationUser
...
from app.bot.kbds.reply import get_keyboard, menu_kb
from app.database.orm_query import orm_Edit_user_profile, orm_Get_info_user

user_view_profile_router = Router()

@user_view_profile_router.message(MainStates.user_view_profile, F.text)
async def change_edit_profile(message: Message, state: FSMContext):
    if message.text.lower() == 'редактировать':
        ...
        await state.set_state(MainStates.user_edit_profile)
...
@user_view_profile_router.message(MainStates.user_edit_profile, F.text)
